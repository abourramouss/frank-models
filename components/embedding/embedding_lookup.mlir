...
module @embedding_components {
  ...
  util.func public @embedding_lookup(
      %weight: tensor<?x?xf16>,   // [vocab_size, n_embd]
    ...
    %n_embd = tensor.dim %weight, %c1 : tensor<?x?xf16>
    ...
    %init_flat = tensor.empty(%batch_seq, %n_embd) : tensor<?x?xf16>
    %embeddings_flat = iree_linalg_ext.gather dimension_map = [0]
        ins(%weight, %indices_flat : tensor<?x?xf16>, tensor<?xi64>)
        outs(%init_flat : tensor<?x?xf16>) -> tensor<?x?xf16>
    ...
    util.return %embeddings : tensor<?x?x?xf16>
  }
  ...
  util.func public @embedding_lookup_1d(
    ...
    %init = tensor.empty(%batch, %n_embd) : tensor<?x?xf16>
    %embeddings = iree_linalg_ext.gather dimension_map = [0]
        ins(%weight, %indices : tensor<?x?xf16>, tensor<?xi64>)
        outs(%init : tensor<?x?xf16>) -> tensor<?x?xf16>

    util.return %embeddings : tensor<?x?xf16>
  }
