...
// Replaces CUDA kernel: 256 threads, 8 rows/block, shared mem, warp shuffle
  ...
  hal.executable.variant public @cuda_nvptx_fb target(<"cuda", "cuda-nvptx-fb", {iree_codegen.target_info = #iree_gpu.target<arch = "sm_87", features = "+ptx74", wgp = <compute = fp64|fp32|fp16|int64|int32|int16|int8, storage = b64|b32|b16|b8, subgroup = shuffle|arithmetic, dot = dp4xi8toi32, mma = [], subgroup_size_choices = [32], max_workgroup_sizes = [1024, 1024, 1024], max_thread_count_per_workgroup = 1024, max_workgroup_memory_bytes = 166912, max_workgroup_counts = [2147483647, 65535, 65535]>>}>) {
    ...
    builtin.module {
      func.func @decode_dispatch_18_matmul_6144x1x1024_f16()
        ...
        // Decode push constants: layer index (i64 from 2×i32)
        ...
        %q8 = hal.interface.binding.subspan layout(<constants = 2, bindings = [
          ...
          #hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">,
          #hal.pipeline.binding<storage_buffer, Indirect>
        ], flags = Indirect>) binding(0) alignment(64) offset(%c0) flags(ReadOnly) : memref<28x6684672xi8, #hal.descriptor_type<storage_buffer>>
        %input = hal.interface.binding.subspan layout(<constants = 2, bindings = [
          #hal.pipeline.binding<storage_buffer, ReadOnly>,
          #hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">,
          #hal.pipeline.binding<storage_buffer, Indirect>
        ], flags = Indirect>) binding(1) alignment(64) offset(%c64) flags("ReadOnly|Indirect") : memref<1024xf16, strided<[1], offset: ?>, #hal.descriptor_type<storage_buffer>>
        %output = hal.interface.binding.subspan layout(<constants = 2, bindings = [
          #hal.pipeline.binding<storage_buffer, ReadOnly>,
          #hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">,
          #hal.pipeline.binding<storage_buffer, Indirect>
        ], flags = Indirect>) binding(2) alignment(64) offset(%c6144_bytes) flags(Indirect) : memref<6144xf16, strided<[1], offset: ?>, #hal.descriptor_type<storage_buffer>>
        ...
        scf.for %i = %tid to %c1024 step %c256 {
          %v = memref.load %input[%i] : memref<1024xf16, strided<[1], offset: ?>, #hal.descriptor_type<storage_buffer>>
          memref.store %v, %smem[%i] : memref<1024xf16, #gpu.address_space<workgroup>>
        }
        gpu.barrier
        ...
        scf.if %in_bounds {
          // Q8_0 byte offset for this row and lane's block
          %row_off = arith.muli %row, %c1088 : index
          %blk_off = arith.muli %lane, %c34 : index
          %byte_off = arith.addi %row_off, %blk_off : index

          // Load scale (2 bytes → f16 → f32)
          %s0 = memref.load %q8[%layer_idx, %byte_off] : memref<28x6684672xi8, #hal.descriptor_type<storage_buffer>>
          %s1_idx = arith.addi %byte_off, %c1 : index
          %s1 = memref.load %q8[%layer_idx, %s1_idx] : memref<28x6684672xi8, #hal.descriptor_type<storage_buffer>>
          ...
          // Dot product: 32 Q8 values × shared memory input
          ...
          %dot = scf.for %j = %c0 to %c32 step %c1 iter_args(%acc = %cst_zero) -> (f32) {
            ...
            %qv = memref.load %q8[%layer_idx, %j_off] : memref<28x6684672xi8, #hal.descriptor_type<storage_buffer>>
          ...
          // Warp shuffle reduction (5 rounds)
          ...
          scf.if %is_lane0 {
            ...
            memref.store %out_f16, %output[%row] : memref<6144xf16, strided<[1], offset: ?>, #hal.descriptor_type<storage_buffer>>
          }
        }
        return
      }
